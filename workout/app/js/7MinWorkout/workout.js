	...
	var startExercise = function (exercisePlan) {
		...
		.then(function () {
			...
			if (next) {
				...
				$location.path('/finish');  //-> navega al path que se indique

				//$location.search() -> se obtiene los parametros de la url
				//$location.hash() -> se obtiene loq ue esta despues del #
				console.log("Workout complete!")
			}
		});
	};
	...
	var createWorkout = function () {
		...
		workout.exercises.push({
              details: new Exercise({
                  ...
                  procedure: "Assume an erect position, with feet together and arms at your side. Slightly bend your knees, and propel yourself a few inches into the air.\
  ...
  var workoutPlanwatch = $scope.$watch('workoutPlan', function
    ...
    $scope.$watch('currentExerciseDuration', function (newValue,oldValue) {
        if (newValue) {
          ...
          if (newValue == $scope.currentExercise.duration / 2 && 
